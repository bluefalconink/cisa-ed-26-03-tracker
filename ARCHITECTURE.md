...
**Auto-generated on 2026-02-27 01:06 UTC** by [generate-docs.yml](.github/workflows/generate-docs.yml)
...
```mermaid
graph TB
    User["🧑‍💻 Agency Analyst<br/>Browser"] --> |HTTPS| Streamlit["🦅 Streamlit App<br/>app.py"]
    Streamlit --> |SQLAlchemy ORM| DB["🗄️ SQLite / PostgreSQL<br/>database.py"]
    Streamlit --> |JSON / CSV Export| Report["📄 CISA Report<br/>March 5 Deadline"]
    Streamlit --> |File Upload| Import["📥 Bulk Import<br/>CSV / Excel / JSON / TSV"]
    Docker["🐳 Docker Container<br/>python:3.11-slim"] --> Streamlit
    GCR["☁️ Google Cloud Run"] --> Docker
    IAP["🔒 Identity-Aware Proxy"] --> GCR
...
| # | Section | Line |
|---|---------|------|
| 1 | 1️⃣ Inventory — All In-Scope Cisco SD-WAN Systems | L110 |
| 2 | 2️⃣ Collect Artifacts → 3️⃣ Patch → 4️⃣ Hunt → 5️⃣ Implement Hardening | L308 |
...
```mermaid
flowchart TD
    A["🏠 Page Load"] --> B["⏰ Deadline Countdowns"]
    B --> C["📊 Dashboard Metrics"]
    C --> D["1️⃣ Asset Inventory"]
    D --> D1["Manual Registration"]
    D --> D2["Bulk Import<br/>CSV/Excel/JSON/TSV"]
    D --> E["2-5️⃣ Compliance Workflow"]
    ...
    E --> F["🔍 Threat Hunt Guide"]
    F --> F1["Attack Chain"]
    F --> F2["CLI Commands"]
    F --> F3["IOC Reference"]
    F --> F4["Decision Flowchart"]
    E --> G["📋 Hunt Findings Log"]
    G --> H["📤 CISA Reporting Engine"]
    H --> H1["JSON Export"]
    H --> H2["CSV Export"]
...
graph LR
    ...
    subgraph "Production (GCP)"
        IAP2["🔒 IAP<br/>Org SSO"] --> CR["☁️ Cloud Run"]
        CR --> CONT["🐳 Docker<br/>python:3.11-slim"]
        CONT --> APP["Streamlit<br/>Port 8080"]
        APP --> PG["🐘 Cloud SQL<br/>PostgreSQL"]
    end
...
- **Exposed Port:** `8080`
- **Entrypoint:** `python -c "import urllib.request; urllib.request.urlopen('http://localhost:8080/_stcore/health')" || exit 1`

## 7. File Inventory

| File | Lines | Purpose |
|------|-------|---------|
| `.github/dependabot.yml` | 35 | GitHub Actions workflow |
| `.github/scripts/generate_architecture.py` | 474 | CI/CD script |
| `.github/workflows/generate-docs.yml` | 42 | GitHub Actions workflow |
| `.github/workflows/security-scan.yml` | 128 | GitHub Actions workflow |
| `=2.0.36` | 14 |  |
| `ACSC-led Cisco SD-WAN Hunt Guide.pdf` | 24340 | Official joint threat hunt guide (PDF) |
| `ARCHITECTURE.md` | 253 | This auto-generated architecture doc |
| `Dockerfile` | 32 | Container build configuration |
...
graph LR
    ...
    MD --> |"git commit + push"| REPO["📦 Repository"]
